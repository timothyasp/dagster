...
def test_default_grpc_timeouts():
    with environ(
        ...
        assert default_schedule_grpc_timeout() == 300
        assert default_sensor_grpc_timeout() == 300
        assert default_grpc_server_shutdown_grace_period() == 300
...
def test_override_grpc_timeouts():
    ...
    with environ(
        {
            "DAGSTER_GRPC_TIMEOUT_SECONDS": "360",
        }
    ):
        assert default_grpc_timeout() == 360
        assert default_schedule_grpc_timeout() == 360
        assert default_sensor_grpc_timeout() == 360
        assert default_grpc_server_shutdown_grace_period() == 360
